...
from deepfake.loggers.buffering_handler import FTBufferingHandler
from deepfake.loggers.ft_rich_handler import FtRichHandler
from deepfake.loggers.rich_console import get_rich_console


# from deepfake.loggers.std_err_stream_handler import FTStdErrStreamHandler
...
FT_LOGGING_CONFIG = {
    ...
    "handlers": {
        "console": {
            "class": "deepfake.loggers.ft_rich_handler.FtRichHandler",
            "formatter": "basic",
        },
    },
    "root": {
...
def _set_log_levels(
    ...
    third_party_loggers = {
        "deepfake": logging.INFO if verbosity <= 1 else logging.DEBUG,
...
def _create_log_config(config: Config) -> dict[str, Any]:
    ...
    for handler_config in log_config.get("handlers", {}).values():
        if handler_config.get("class") == "deepfake.loggers.ft_rich_handler.FtRichHandler":
            ...
            try:
                ...
                raise OperationalException(
                    ...
                    "directories. If you're running deepfake using docker, you see this error "
